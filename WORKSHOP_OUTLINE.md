...
1. **Locate the IIIF Manifest Link**:
   - On the David Rumsey map record page, look for an **"IIIF"** button or link
   - This is typically located in the object information panel (often on the right side or below the map image)
   - Click it to reveal the IIIF manifest URL
...
3. **Confirm the Map Loaded**:
   ...
   - You should see zoom and pan controls
   - On the right side, you'll see an empty **Ground Control Points (GCPs)** panel (where you'll add control points)
...
**Objective**: Define which parts of the image represent the actual map (and exclude borders, margins, labels)
...
2. **Access Mask Tool**:
   - Look for a "Mask" or "Create Mask" option in the editor interface
   - This typically opens a drawing tool overlaid on the map

3. **Draw the Mask**:
   - Use the polygon or freehand drawing tool to outline the map area
...
4. **Save/Confirm the Mask**:
   - Once satisfied, confirm or save the mask
...
1. **Start Adding Control Points**:
   - Click the "Add GCP" button or similar in the editor
   - You now have two panels: the historical map on the left, and a modern reference map on the right (or both in the same view)

2. **Click on the Historical Map**:
   - Identify a clear, identifiable feature (street intersection, building corner, bridge, prominent landmark)
   - Click that exact point on the historical map in the left panel
   - A crosshair or marker will appear
